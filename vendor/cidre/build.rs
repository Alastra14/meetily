...
fn xc_build(
    ...
    // Ternova Meet: si no hay Xcode completo (solo Command Line Tools), compilar
    // los shims ObjC de pomace directamente con clang/ar. Mismo layout de salida
    // que xcodebuild (SYMROOT/<configuration>/lib<target>.a). Con Xcode presente
    // el comportamiento es idéntico al upstream.
    let xcodebuild_ok = Command::new("xcodebuild")
        .arg("-version")
        .output()
        .map(|o| o.status.success())
        .unwrap_or(false);

    let status = if !xcodebuild_ok {
        assert!(
            sdk == "macosx",
            "clang fallback for pomace only supports macOS (sdk={sdk}); install full Xcode"
        );
        clang_build_pomace(targets, arch, configuration, &out_lib_dir)
...
/// Ternova Meet — compila cada target de pomace (`./pomace/<t>/<t>.m`) con clang
/// de Command Line Tools y lo archiva como `lib<t>.a`, replicando el producto de
/// xcodebuild. Los headers de pomace contienen las definiciones (Class X; y
/// funciones), así que un único .o por módulo produce todos los símbolos que
/// cidre enlaza con #[link(name = "<t>", kind = "static")].
fn clang_build_pomace(
    targets: &[&'static str],
    arch: &str,
    configuration: &str,
    out_lib_dir: &PathBuf,
    ...
    let sdk_path_out = Command::new("xcrun")
        ...
        .output()
        .expect("xcrun --show-sdk-path failed (Command Line Tools required)");
    assert!(sdk_path_out.status.success(), "xcrun --show-sdk-path failed");
    let sdk_path = String::from_utf8(sdk_path_out.stdout)
        .unwrap()
        .trim()
        .to_string();

    let libs_dir = out_lib_dir.join(configuration);
    std::fs::create_dir_all(&libs_dir).unwrap();

    let mut include_dirs: Vec<String> = vec!["./pomace".to_string()];
    if let Ok(rd) = std::fs::read_dir("./pomace") {
        for e in rd.flatten() {
            if e.path().is_dir() {
                if let Some(n) = e.file_name().to_str() {
                    include_dirs.push(format!("./pomace/{n}"));
                }
            }
        }
    }

    let mut last: Option<std::process::ExitStatus> = None;
    let mut i = 0;
    while i < targets.len() {
        if targets[i] == "-target" && i + 1 < targets.len() {
            let name = targets[i + 1];
            let src = format!("./pomace/{name}/{name}.m");
            let obj = libs_dir.join(format!("{name}.o"));
            let lib = libs_dir.join(format!("lib{name}.a"));

            let mut cc = Command::new("clang");
            cc.args(["-c", "-fobjc-arc", "-fmodules", "-x", "objective-c"])
                .args(["-isysroot", &sdk_path])
                .args(["-arch", arch])
                .args(["-mmacosx-version-min=13.0", "-O2", "-Wno-everything"]);
            for d in &include_dirs {
                cc.arg("-I").arg(d);
            }
            let st = cc.arg(&src).arg("-o").arg(&obj).status().unwrap();
            assert!(st.success(), "clang failed for pomace target {name}");
            ...
            let st = Command::new("ar")
                ...
                .status()
                .unwrap();
            assert!(st.success(), "ar failed for pomace target {name}");
            last = Some(st);
            i += 2;
        } else {
            i += 1;
    ...
    last.expect("no pomace targets requested")
}
