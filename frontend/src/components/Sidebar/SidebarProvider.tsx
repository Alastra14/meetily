...
export function SidebarProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchMeetings = React.useCallback(async () => {
    if (serverAddress) {
      try {
        const meetings = await invoke('api_get_meetings') as Array<{ id: string, title: string, source?: string | null }>;
  ...
  const baseItems: SidebarItem[] = [
    {
      ...
      children: [
        ...meetings.map(meeting => ({ id: meeting.id, title: meeting.title, type: 'file' as const, source: meeting.source ?? null }))
      ]
    },
  ];
